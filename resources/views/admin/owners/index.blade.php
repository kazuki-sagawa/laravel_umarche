            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="md:p-6 bg-white border-b border-gray-200">
                                ...
                                <tbody>
                                    @foreach ($owners as $owner)
                                        <tr>
                                            ...
                                            <td class="md:px-4 py-3">{{ $owner->create_at->diffForHumans() }}</td>
                                            <td class="md:px-4 py-3">
                                                <button onclick="location.href='{{ route('admin.owners.edut', ['owner' => $owner->id]) }}'" class="text-white bg-yellow-400 border-0 py-2 px-4 focus:outline-none hover:bg-yellow-600 rounded">編集</button>
                                            </td>
                                            <form id="delete_{{ $owner->id }}" method="post" action="{{ route('admin.owners.destroy', ['owner' => $owner->id]) }}">
                                                @csrf
                                                @method('delete')
                                                <td class="px-4 py-3">
                                                    <a href="#" deta-id="{{ $owner->id }}" onclick="deletePost(this)" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-600 rounded">削除</a>
                                                </td>
                                            </form>
                                        </tr>
                                    @endforeach
                    ...
                    @foreach ($e_all as $e_owner)
                        {{ $e_owner->name }}
                        {{ $e_owner->create_at->diffForHumans() }}
                    @endforeach
                    <br />
                    クエリビルダ
                    @foreach ($q_get as $q_owner)
                        {{ $q_owner->name }}
                        {{ Carbon\Carbon::parse($q_owner->create_at)->diffForHumans() }}
                    @endforeach -->
                </div>
            </div>
        </div>
